...
Новые сообщения приходят в интерфейс через SSE и сразу добавляются в открытый чат. После события frontend тихо перечитывает историю и список диалогов, поэтому порядок, превью и непрочитанные сообщения синхронизируются с Telegram. Аватары и вложения загружаются лениво; файлы сообщений кэшируются в `MEDIA_CACHE_DIR` и ограничиваются `MAX_MEDIA_BYTES`.
